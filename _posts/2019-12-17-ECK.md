...
Therefore Entropy can be defined as \\(-log(A)\\). But, in machine learning we use Expectation of an event therefore multiply it with the probability of the event. Therefore, Expectation of entropy is \\[= -A.log(A)\\].  

### Cross-Entropy
It is a way of measuring the distance or similarity between two distribution and in the Machine Learnig case Probability distribution.