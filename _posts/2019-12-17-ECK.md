...
If two distributions perfectly match, \\(D(p,q) = 0\\) otherwise it can take values between 0 and ∞. Lower the KL divergence value, the better we have matched the true distribution with our approximation. Since KL divergence is gain or loss of information when we move between probability distribution p and q. Let's say we start wth some random distribution **q** then then value of KL divergence will be very high. 
