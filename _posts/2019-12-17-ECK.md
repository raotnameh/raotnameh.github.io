...
and in log space. \\[LR = \sum\limits_{i=1}^n \log (\frac{p(x_i)}{q(x_i)})\\]
...
Simplifying the above expression gives us fuck: \\[D(p,q) = \sum\limits_{i=1}^n p(x_i) * \log p(x_i)} - \sum\limits_{i=1}^n p(x_i) * \log q(x_i)}\\]  
print(m)