...
Therefore Entropy can be defined as \\(-log(A)\\). But, in machine learning we use Expectation of an event therefore multiply it with the probability the event. Therefore, Expectation of entropy is \\[= -A*log(A)\\].