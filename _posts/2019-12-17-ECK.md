...
But, in Machine Learning (ML) we use Expectation of an event therefore multiply it with the probability of the event. Therefore, Expectation of uncertainity is:  \\[-A * log(A)\\]. Expectation of uncertainity is Entropy or Entropy is average Information that we get, when a random sample is drawn from a given event.
