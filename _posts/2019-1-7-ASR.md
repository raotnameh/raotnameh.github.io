...
<br />
<br />
what do I mean by all the other PDF's? Let's take an example of gaussian distribution. why? because it will be easy for us in the math department. The gaussian distribution formualtion and it's plot can be seen in Fig. 1. As shown in fig for different values of \\(\\mu \\) and \\(\\sigma \\) we can get different plots or I would say different PDF's. therefore one each distribution produces a set of data or inversley, some specific data can produce a PDF from a family of PDF's.
