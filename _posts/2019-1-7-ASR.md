...
<br />
* \\(P(O / W) \\): It means, given the observed word sequence W, the one Probability density function (PDF) \\(f(O / \\theta) \\) among all the PDF's that are most likely to have produced the data. \\(\\theta\\) is a learnable parameter.
<br />
<br />
