...
The acoustic model is responsible for calculating the \\(P(O / W) \\) and the LM calculates the \\(P(W) \\). Language model plays an important role, when there is ambguity in the predicted text. for example, **I *____* you**, if you have to choose between **_NO_** and **_KNOW_** which one whould you choose?
Of course the second one, but why ? I would say, you have language understanding as one of your cognitive skills,that's how you  did it. Therefore, LM helps in making sense for a given word sequence.  <br /> 
<br />
