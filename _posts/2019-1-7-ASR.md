...
WER: It's computed using "String edit distance" or "Levenstein distance." It is \\[WER = \frac{N<sub>sub</sub> + N<sub>ins</sub> + N<sub>del</sub>}{N<sub>ref</sub>}\\]. Where,  
...
CER: Everythinng is sameexcept instead of WRODS it considers __CHARACTERS__ for all the calcualtions in the above formula.  
SER: It's very simple to calculate. according tothe SER defination, even if a single word in a sentence is incorrect it wil label it as incorrect hypothesis. SER is used as _proportion of incorect sentences to total number of sentences_
