...
The acoustic model is responsible for calculating the \\(P(O / W) \\) and the Language model calculates the \\(P(W) \\) come into the picture to make sense out of predicted word sequence. Language model also plays an importatn role when there is ambguity in the predictted text. for example, I *blank* you, if you have to choose between _NO_ and _KNOW_ which one whould you choose?
