...
+ Spectrogram : It's a better representation of an Audio signal, to fed to an Neural Network.
+ Acoustic Model : Acoustic models calculates/determines the probabiltiy of output conditioned on an input. The input is either Raw audio or spectrogram and the output is the predicted transcription. 
### STEP 2
